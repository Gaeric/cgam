...
class SoftBody {
  ...
  initPhysics() {
    ...
    for (var i = 0; i < this.edgeLengths; i++) {
...
class Grabber {
  ...
  updateRaycaster(x, y) {
    ...
    this.mousePos.x = ((x - rect.lect) / rect.width) * 2 - 1;
  ...
  move(x, y) {
    if (this.physicsObject) {
      this.updateRaycaster(x, y);
      var pos = this.raycaster.ray.origin.clone();
      pos.addScaledVector(this.raycas.ray.direction, this.distance);

      this.vel.copy(pos);
      this.vel.sub(this.prevPos);
      if (this.time > 0.0) {
        this.vel.divideScale(this.time);
      } else {
        this.vel.set(0.0, 0.0, 0.0);
      }
...
function onPointer(evt) {
  ...
  } else if (evt.type == "pointremove" && gMouseDown) {
